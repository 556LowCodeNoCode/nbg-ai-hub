...
**Last updated:** 2026-05-29 (migration + use-cases v4) — **Repo migrated to 556LowCodeNoCode/NbgAiHub. Site now serves at <https://556lowcodenocode.github.io/NbgAiHub/>. + Self-sufficient use cases + skip-permissions default + OS picker inside Step 1 + CLAUDE.md teaching across all 12.** The 12 use cases now work end-to-end for a bank colleague with **zero system access** — every prior assumption of a real CSV/PDF/DOCX/Teams-transcript has been replaced with a "Step 2 — ask Claude to invent a realistic synthetic [X]". The real-data swap-in is a one-line note ("once you trust the loop, swap your real export into the same filename"). `claude --dangerously-skip-permissions` is the project default invocation (documented once per use case with an opt-out for first-timers). The OS picker moved from the hero (where it was being overlooked) to a prominent accent-bordered panel at the top of Step 1's prose column; the hero's existing OS toggle was removed. Each Step 1 Windows div now carries a short paragraph explaining how `~/Desktop` resolves inside WSL (`/home/<linux-user>/Desktop`, not the Windows desktop) and pointing readers at `explorer.exe .` as the one-line bridge to Windows Explorer. Every use case's final step now closes with a CLAUDE.md teaching subsection — opens with *"CLAUDE.md is the magic filename Claude Code reads automatically every time you start `claude` in a folder containing it"*, shows what stable rules go in it, shows how next week's run shrinks to a one-liner. `complaint-heatmap` (the first by order) is the only use case that also introduces global `~/.claude/CLAUDE.md` with a three-line banking-colleague starter. The picker's button-pair was misaligned by 8px — Starlight's unlayered `* + *` rule had pushed the second button down; fixed with `margin: 0 !important` on `.os-toggle__btn` plus `min-width: 7.25rem` + `justify-content: center` + 18×18 SVG normalisation so the segmented pair looks structurally identical regardless of label length or selection state. End-to-end verification: every use case ran in `/tmp/nbg-usecase-tests/` producing realistic outputs; the `sql-from-question` Step-4 verification step caught a real Postgres typing bug in the first-pass query — the pedagogy works. Site live at <https://556lowcodenocode.github.io/NbgAiHub/>. AUTO doc counts (canonical table below): glossary 45, tips 27, skills 6, use cases 12, journeys 2, news 52 — the docs-drift workflow and `scripts/sync-doc-counts.mjs` now both track use cases as a first-class pillar.
